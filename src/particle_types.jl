...
!!! warning
    In the current implementation there are **no checks** build-in, which verify the passed arguments whether they describe on-shell particles, i.e. `p*p≈mass^2`. Using `base_state` with off-shell particles will cause unpredictable behavior.
"""
...
function base_state(
    ...
    return BiSpinor(booster[:, spin_index(spin)])
end
...
function base_state(
    ...
    return AdjointBiSpinor(BiSpinor(booster[:, spin_index(spin)+2])) * GAMMA[1]
end
...
function base_state(
    ...
    booster = _booster_antifermion(mom, mass(particle))
    return SVector(AdjointBiSpinor(BiSpinor(booster[:, 3])) * GAMMA[1],
...
function base_state(
    ...
    booster = _booster_fermion(mom, mass(particle))
    return AdjointBiSpinor(BiSpinor(booster[:, spin_index(spin)])) * GAMMA[1]
end
...
function base_state(
    ...
    booster = _booster_fermion(mom, mass(particle))
    return SVector(AdjointBiSpinor(BiSpinor(booster[:, 1])) * GAMMA[1],
...
function base_state(
    ...
    booster = _booster_antifermion(mom, mass(particle))
    return BiSpinor(booster[:, spin_index(spin)+2])
end
